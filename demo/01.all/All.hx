...
class All extends doom.html.Component<{}> {
  override function render() {
    return div(["class" => "container"], [
      ...
      row2(
        ...
        [],
        [
          alertSuccess("a permanent alert").asNode(),
          alertDanger({ dismissable : true }, "a dismissable alert").asNode()
        ]
      ),
      // row2(
      //   radioButtons(),
  ...
  public function buttons() {
    return section("Buttons", [
      buttonPrimary(onClick, "Primary button").asNode(),
      buttonSecondary(onClick, "Secondary button").asNode(),
      buttonInfo(onClick, "Info button").asNode(),
      buttonSuccess(onClick, "Success button").asNode(),
      buttonWarning(onClick, "Warning button").asNode(),
      buttonDanger(onClick, "Danger button").asNode(),
    ]);
  }
